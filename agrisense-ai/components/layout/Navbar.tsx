...
import type { MqttStatus } from "@/lib/websocket";
...
export default function Navbar() {
  ...
  const mqttStatus = useAgriStore((s) => s.mqttStatus);
  const [showNotif, setShowNotif] = useState(false);
  const [mobileOpen, setMobileOpen] = useState(false);
  const [unread, setUnread] = useState(2);

  const statusCfg = mqttStatusConfig[mqttStatus];
        ...
        <div className="flex items-center justify-between px-5 py-3">
          ...
          <Link href="/" className="flex items-center gap-2.5 group flex-shrink-0">
            ...
            <div className="leading-none">
              <div className="font-bold text-base tracking-tight" style={{ color: isDark ? "#e8f0eb" : "#1e2b22" }}>
                AgriSense
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="hidden md:block">{statusCfg.label}</span>
              {statusCfg.pulse && (
                <span
            ...
            <div className="relative">
              <button
                ...
                style={{
                  background: showNotif ? "rgba(47,158,68,0.15)" : "rgba(47,158,68,0.08)",
                  color: "#2f9e44",
                }}
              >
                ...
                {showNotif && (
                  ...
                  >
                    ...
                    >
                      ...
                      <button onClick={() => setShowNotif(false)} style={{ color: "#6b7c72" }}>
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                    <div className="divide-y" style={{ borderColor: isDark ? "rgba(47,158,68,0.1)" : "#d9e5dc" }}>
                      {mockNotifications.map((n) => (
                        <div
                          key={n.id}
                          className="flex items-start gap-3 px-4 py-3 hover:bg-green-50 dark:hover:bg-green-900/10 transition-colors cursor-pointer"
                        >
                          <div className="mt-0.5">{notifIcons[n.type]}</div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate" style={{ color: isDark ? "#e8f0eb" : "#1e2b22" }}>
                              {n.title}
                            </p>
                ...
                {navItems.map(({ label, href, icon: Icon }) => {
                  ...
                  return (
                    <Link
                      ...
                      style={{
                        color: active ? "#2f9e44" : isDark ? "#9bb8a4" : "#6b7c72",
                        background: active ? "rgba(47,158,68,0.1)" : "transparent",
                      }}
                    >
                      <Icon className="w-3.5 h-3.5" />
                      {label}
